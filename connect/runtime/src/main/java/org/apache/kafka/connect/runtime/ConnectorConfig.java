...
public class ConnectorConfig extends AbstractConfig {
    ...
    private static final String CONNECTOR_CLASS_DOC = "Name or alias of the class for this connector. Must be a subclass of org.apache.kafka.connect.connector.Connector. "
            +
    ...
    // The Connector config should not have a default for the header converter,
    // since the absence of a config property means that
    // the worker config settings should be used. Thus, we set the default to null
    // here.
    ...
    private static final String CONFIG_RELOAD_ACTION_DOC = "The action that Connect should take on the connector when changes in external "
            +
    ...
    public static final String CONFIG_RELOAD_ACTION_NONE = Herder.ConfigReloadAction.NONE.name()
            .toLowerCase(Locale.ROOT);
    public static final String CONFIG_RELOAD_ACTION_RESTART = Herder.ConfigReloadAction.RESTART.name()
            .toLowerCase(Locale.ROOT);
    ...
    public static ConfigDef configDef() {
        ...
        return new ConfigDef()
                .define(NAME_CONFIG, Type.STRING, ConfigDef.NO_DEFAULT_VALUE, nonEmptyStringWithoutControlChars(),
                        Importance.HIGH, NAME_DOC, COMMON_GROUP, ++orderInGroup, Width.MEDIUM, NAME_DISPLAY)
                .define(CONNECTOR_CLASS_CONFIG, Type.STRING, Importance.HIGH, CONNECTOR_CLASS_DOC, COMMON_GROUP,
                        ++orderInGroup, Width.LONG, CONNECTOR_CLASS_DISPLAY)
                .define(TASKS_MAX_CONFIG, Type.INT, TASKS_MAX_DEFAULT, atLeast(TASKS_MIN_CONFIG), Importance.HIGH,
                        TASKS_MAX_DOC, COMMON_GROUP, ++orderInGroup, Width.SHORT, TASK_MAX_DISPLAY)
                ...
                .define(TRANSFORMS_CONFIG, Type.LIST, Collections.emptyList(),
                        ConfigDef.CompositeValidator.of(new ConfigDef.NonNullValidator(), new ConfigDef.Validator() {
                        ...
                        }), Importance.LOW, TRANSFORMS_DOC, TRANSFORMS_GROUP, ++orderInGroup, Width.LONG,
                        TRANSFORMS_DISPLAY)
                .define(CONFIG_RELOAD_ACTION_CONFIG, Type.STRING, CONFIG_RELOAD_ACTION_RESTART,
                        ...
                        CONFIG_RELOAD_ACTION_DOC, COMMON_GROUP, ++orderInGroup, Width.MEDIUM,
                        CONFIG_RELOAD_ACTION_DISPLAY)
                .define(ERRORS_RETRY_TIMEOUT_CONFIG, Type.LONG, ERRORS_RETRY_TIMEOUT_DEFAULT, Importance.MEDIUM,
                        ERRORS_RETRY_TIMEOUT_DOC, ERROR_GROUP, ++orderInErrorGroup, Width.MEDIUM,
                ...
                .define(ERRORS_RETRY_MAX_DELAY_CONFIG, Type.LONG, ERRORS_RETRY_MAX_DELAY_DEFAULT, Importance.MEDIUM,
                        ERRORS_RETRY_MAX_DELAY_DOC, ERROR_GROUP, ++orderInErrorGroup, Width.MEDIUM,
                ...
                .define(ERRORS_LOG_INCLUDE_MESSAGES_CONFIG, Type.BOOLEAN, ERRORS_LOG_INCLUDE_MESSAGES_DEFAULT,
                        Importance.MEDIUM,
                        ERRORS_LOG_INCLUDE_MESSAGES_DOC, ERROR_GROUP, ++orderInErrorGroup, Width.SHORT,
    ...
    /**
     * Returns the initialized list of {@link Transformation} which are specified in
    ...
    /**
     * Returns an enriched {@link ConfigDef} building upon the {@code ConfigDef},
     * using the current configuration specified in {@code props} as an input.
     * <p>
     * {@code requireFullConfig} specifies whether required config values that are
     * missing should cause an exception to be thrown.
     */
    public static ConfigDef enrich(Plugins plugins, ConfigDef baseConfigDef, Map<String, String> props,
        ...
        for (Object o : uniqueTransformAliases) {
            ...
            newDef.define(transformationTypeConfig, Type.CLASS, ConfigDef.NO_DEFAULT_VALUE, typeValidator,
                    Importance.HIGH,
                    "Class for the '" + alias + "' transformation.", group, orderInGroup++, Width.LONG,
                    "Transformation type for " + alias,
    ...
    /**
     * Return {@link ConfigDef} from {@code transformationCls}, which is expected to
     * be a non-null {@code Class<Transformation>},
    ...
    static ConfigDef getConfigDefFromTransformation(String key, Class<?> transformationCls) {
        ...
        if (Modifier.isAbstract(transformationCls.getModifiers())) {
            ...
            String message = childClassNames.trim().isEmpty() ? "Transformation is abstract and cannot be created."
        ...
        try {
            ...
            ConfigException exception = new ConfigException(key, String.valueOf(transformationCls),
